...
(* ------------------------------------------------------------------------- *)
  ...
  let dest_thm (Sequent(_,asl,c)) = (asl,c)

  let hyp (Sequent(_,asl,c)) = asl

  let concl (Sequent(_,asl,c)) = c
...
(* ------------------------------------------------------------------------- *)
  ...
  let REFL tm =
    mk_sequent_0 ("REFL", [], (safe_mk_eq tm tm))

  let TRANS (Sequent(hi1,asl1,c1)) (Sequent(hi2,asl2,c2)) =
    match (c1,c2) with
      Comb((Comb(Const("=",_),_) as eql),m1),Comb(Comb(Const("=",_),m2),r)
        when alphaorder m1 m2 = 0 -> (mk_sequent_2 hi1 hi2 ("TRANS", term_union asl1 asl2, Comb(eql,r)))
    | _ -> failwith "TRANS"
  ...
  let BETA tm =
    match tm with
      Comb(Abs(v,bod),arg) when Pervasives.compare arg v = 0
        -> mk_sequent_0("BETA",[],safe_mk_eq tm bod)
...
(* ------------------------------------------------------------------------- *)
  ...
  let ASSUME tm =
    if Pervasives.compare (type_of tm) bool_ty = 0 then mk_sequent_0("ASSUME",[tm],tm)
    else failwith "ASSUME: not a proposition"

  let EQ_MP (Sequent(hi1, asl1,eq)) (Sequent(hi2, asl2,c)) =
    match eq with
      Comb(Comb(Const("=",_),l),r) when alphaorder l c = 0
        -> mk_sequent_2 hi1 hi2 ("EQ_MP", term_union asl1 asl2,r)
    | _ -> failwith "EQ_MP"

  let DEDUCT_ANTISYM_RULE (Sequent(hi1, asl1, c1)) (Sequent(hi2, asl2, c2)) =
    let asl1' = term_remove c2 asl1 and asl2' = term_remove c1 asl2 in
    mk_sequent_2 hi1 hi2 ("DEDUCT_ANTISYM_RULE", term_union asl1' asl2',safe_mk_eq c1 c2)

(* ------------------------------------------------------------------------- *)
(* Type and term instantiation.                                              *)
(* ------------------------------------------------------------------------- *)

  let INST_TYPE theta (Sequent(hi, asl,c)) =
    let inst_fn = inst theta in
    mk_sequent_1 hi ("INST_TYPE",term_image inst_fn asl,inst_fn c)

  let INST theta (Sequent(hi,asl,c)) =
    let inst_fun = vsubst theta in
    mk_sequent_1 hi ("INST",term_image inst_fun asl,inst_fun c)
...
(* ------------------------------------------------------------------------- *)
  ...
  let new_axiom tm =
    if Pervasives.compare (type_of tm) bool_ty = 0 then
      let th = mk_sequent_0("AXIOM",[],tm) in
    ...
    match tm with
      Comb(Comb(Const("=",_),Var(cname,ty)),r) ->
        ...
        then failwith "new_definition: Type variables not reflected in constant"
             ...
             let dth = mk_sequent_0("BASIC_DEF",[],safe_mk_eq c r) in
...
(* ------------------------------------------------------------------------- *)
  ...
  let new_basic_type_definition tyname (absname,repname) (Sequent(hi, asl,c)) =
...
let string_label = Hiproofs.make_string_label;;

let hilabel_thm l th = hilabel true l (fun _ -> th) [];;
let hilabel_thm_str tag th = hilabel true (string_label tag) (fun _ -> th) [];;
...
let rich_label, dest_rich_label = 
  ...
  (Hiproofs.make_label rich_label_constr,
   Hiproofs.dest_label rich_label_constr);;
...
let hilabel_tac_goals goal_labels tac g = 
  let (inst, gls, j) = tac g in
  (inst, gls, (fun inst ths -> j inst (List.map2 hilabel_thm goal_labels ths)));;

let hilabel_tac_frame label tac g =
  let (inst, gls, j) = tac g in
  (inst, gls, (fun inst ths -> hilabel_thm label (j inst ths)));;

let hilabel_tac_p label tac p = hilabel_tac false label (tac p);;
