...
let BETAS_CONV tm = 
  hilabel_thm_str "BETAS_CONV" (BETAS_CONV tm);;
...
let INSTANTIATE inst th = 
  hilabel (string_label "INSTANTIATE") 
    (fun ths -> INSTANTIATE inst (List.hd ths)) [th];;

let INSTANTIATE_ALL inst th = 
  hilabel (string_label "INSTANTIATE_ALL") 
    (fun ths -> INSTANTIATE_ALL inst (List.hd ths)) [th];;
...
let new_definition tm =
  ...
  store_thm (Some "DEF") (itlist GEN rvs (itlist GEN avs th2));;
