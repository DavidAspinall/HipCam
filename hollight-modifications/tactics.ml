...
(* ------------------------------------------------------------------------- *)
(* Hiproofs for tactics                                                      *)
(* ------------------------------------------------------------------------- *)
                  
let hilabel_tac label tac g =
  let (inst, gls, j) = tac g in   
  (inst, gls, (fun inst -> hilabel label (j inst)));;

let hilabel_tac_str tag = hilabel_tac (string_label tag);;
...
let REPEAT tac = hilabel_tac_str "REPEAT" (REPEAT tac);;
...
let (EQ_TAC: tactic) =
  hilabel_tac_goals [string_label "==>"; string_label "<=="]
    (hilabel_tac_str "Equivalence"
       (fun (asl,w) ->
         try let l,r = dest_eq w in
...
let STRIP_TAC = hilabel_tac (string_label "STRIP_TAC") STRIP_TAC;;
